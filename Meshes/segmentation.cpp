...
void PixelSegmentation::load_from_file(ifstream& labels_file, PixelSegmentation& s) {
    // format: two int32s specifying width and height, followed by w*h int32s.
    int32_t* dims = new int32_t[2];
    labels_file.read((char*)dims, sizeof(int32_t)*2);
    int32_t width = dims[0];
    ...
    labels_file.read((char*)data, width*height*sizeof(int32_t));
    labels_file.read((char*)bdata, width*height*sizeof(uchar));
    s = PixelSegmentation(data, bdata, width, height);
}
...
void PixelSegmentation::compute_mean (cv::Mat& image, cv::Mat& output)
{
    if ( image.channels() == 1 )
    {
        vector<double> means(this->number_segments(), 0.0);
        for(int p_y = 0; p_y < image.rows; ++p_y) {
            for(int p_x = 0; p_x < image.cols; ++p_x) {
                means[this->segmentation_data.at<int32_t>(p_y, p_x) - 1] += image.at<uchar>(p_y, p_x);
            }
        }
        for(int i=0; i<means.size(); ++i)
        {
            means[i] /= this->segments[i].area();
            //means[i] = int( rand() * 255 );
            //std::cout<<" m["<<i<<"]="<<means[i];
        }
        output = cv::Mat( image.rows, image.cols, CV_8U );
        for( int p_y = 0; p_y < image.rows; ++p_y ) {
            for(int p_x = 0; p_x < image.cols; ++p_x) {
                output.at<uchar>(p_y, p_x) = means[this->segmentation_data.at<int32_t>(p_y, p_x) - 1];
            }
        }
    }
    else if ( image.channels() == 3 )
    {
        ...
        output = cv::Mat( image.rows, image.cols, CV_8UC3 );
        for( int p_y = 0; p_y < image.rows; ++p_y ) {
            for(int p_x = 0; p_x < image.cols; ++p_x) {
...
double PixelSegmentation::reconstruction_error(const cv::Mat& image) {
    ...
    if ( image.channels() == 1 ) greyscale_image = image;
    else cv::cvtColor( image, greyscale_image, COLOR_BGR2GRAY );

    std::vector<double> colours;
    colours.reserve( this->number_segments() );
    ...
    for(int label=1; label <= this->number_segments(); ++label) {
        ...
        colours.push_back( ( sum / 255.0 ) / pixel_count );
    }
    ...
    return sqrt( total_error / image.total() ) * 100; // percents
}
