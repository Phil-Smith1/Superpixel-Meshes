...
double compute_dist (labxy& px, labxy& center, double distance_weight) {
    double d_lab = norm(px.color - center.color);
    double d_xy = norm(px.point - center.point);
    double d = pow(d_lab, 2) + pow( distance_weight * d_xy, 2 );
    return d;
}
...
PixelSegmentation run_slic(Mat& image, int target_superpixel_number, int m) {
    ...
    int step = int( sqrt(w*h / double(target_superpixel_number)) ); // typical side length
    //std::cout<<" w="<<w<<" h="<<h<<" step="<<step;
